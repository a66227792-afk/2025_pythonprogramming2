...
class Bullet(pygame.sprite.Sprite): # 추가: Enemy Sprite 클래스 (빨간 박스 위치에 적 배치)
  ...
  def shoot(self):
    ...
    if keys[pygame.K_SPACE]:
      self.rect.centerx += self.speed_y
...
def spawn_apple():
  ...
  apples.append({"rect": rect, "vy": vy})   #추가: 리스트에 사과 추가
  ...
  for event in pygame.event.get():
    ...
    if event.type == pygame.KEYDOWN:
      if game_over and event.key == pygame.K_RETURN:
        ...
        #추가: 적(똥) 초기화
        bullet.rect.topleft = (WIDTH//2, 260)
        bullet.speed_y = 2
